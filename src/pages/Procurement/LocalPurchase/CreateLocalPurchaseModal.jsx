...
const CreateLocalPurchaseModal = ({
  open,
  setOpen,
  localPurchaseData,
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await api.get("/api/stakeholders/type/28");
      let items = res.data;
      ...
      console.error("Failed to fetch suppliers by type 28", error);
      setSuppliers([]);
    }
  };
  ...
  useEffect(() => {
    if (localPurchaseData) {
      setFormData({
        stakeholderId: localPurchaseData.stakeholderId || localPurchaseData.supplierId || "",
        typeId: 28,
        seedTypeId: localPurchaseData.seedTypeId || "",
        orderedQuantity: localPurchaseData.orderedQuantity || "",
        unitPrice: localPurchaseData.unitPrice || "",
        orderDate: localPurchaseData.orderDate ? localPurchaseData.orderDate.split("T")[0] : "",
        expectedDeliveryDate: localPurchaseData.expectedDeliveryDate ? localPurchaseData.expectedDeliveryDate.split("T")[0] : "",
        otherCharges: localPurchaseData.otherCharges || "",
        notes: localPurchaseData.notes || "",
      });
    } else {
  ...
  const handleClose = () => {
    setOpen(false);
    if (setLocalPurchaseData) setLocalPurchaseData(null);
    setFormData(initialForm);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await Swal.fire({
      title: localPurchaseData ? "Update Local Purchase?" : "Create Local Purchase?",
      text: localPurchaseData
        ? "Do you want to update this local purchase?"
        : "Do you want to create this local purchase?",
      icon: "question",
      showCancelButton: true,
      confirmButtonColor: "#0D9488",
      cancelButtonColor: "#d33",
      confirmButtonText: localPurchaseData ? "Yes, Update" : "Yes, Create",
    });
    ...
    const res = localPurchaseData
      ? await updatePreOrder(localPurchaseData.id, payload)
  ...
  return (
    <CustomModal
      open={open}
      setOpen={handleClose}
      header={localPurchaseData ? "Edit Local Purchase" : "Create Local Purchase"}
      width="w-[60vw]"
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 border-t pt-5">
          ...
          >
            {loading ? "Saving..." : localPurchaseData ? "Update Local Purchase" : "Create Local Purchase"}
          </button>
        </div>
      </form>
    </CustomModal>
...
export default CreateLocalPurchaseModal;
